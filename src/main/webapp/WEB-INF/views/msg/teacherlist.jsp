...
<script type="text/javascript">
	...
	function msgDtl(p){
		...
		let url = "${CONTEXT_PATH}/msg/teacher/detail?msgNo=" + p;
...
// 			 console.log(xhr.responseText);
		     ...
		     let date = new Date(data.msgSntDt);
        ...
        <div class="col-sm-12">
            <div class="card">
                <div class="card-header pb-0">
                    ...
                    <br>
                    <!-- 검색 -->
				  	<form id="msgfrm" action="${CONTEXT_PATH}/msg/teacher/list" method="post">
                ...
                <div class="card-body">
                    <div id="batchDelete" class="jsgrid" style="position:relative; height:auto; width:100%;">
						...
						<form action="${CONTEXT_PATH}/msg/teacher/allDelete" method="post" style="display:none;">
							   ...
							   <div class="modal-content">
							    ...
							    <!-- 모달 머리 -->
							     ...
							     <div class="modal-footer">
							        <form action="${CONTEXT_PATH}/msg/teacher/delete" method="POST" style="display: none;">
                        ...
                        <div class="jsgrid-pager-container" style="text-align: center;" id="pagingCtr">
                            <div class="jsgrid-pager" id="pagingDiv">
                                ...
                                <c:if test="${p.nowPage ne 1}">
  		                       	<span class="jsgrid-pager-nav-button"><a href="${CONTEXT_PATH}/msg/teacher/list?nowPage=1&keyword=${p.keyword}&type=${p.type}">처음</a></span>
  		                       	</c:if>
  		                       	
                            	<!-- 이전 -->
                                <c:if test="${p.nowPage ne 1}">
	                            	<span class="jsgrid-pager-nav-button jsgrid-pager-nav-inactive-button">
	                            		<a href="${CONTEXT_PATH}/msg/teacher/list?nowPage=${p.nowPage - 1}&keyword=${p.keyword}&type=${p.type}">이전</a>
	                            	</span>
                                </c:if>
	                            ...
	                            <c:forEach begin="${p.startPage}" end="${p.endPage}" var="num" step="1">
	                                ...
	                                <c:if test="${num ne p.nowPage}">
	                                	<span class="jsgrid-pager-page" style="cursor:pointer">
	                                		<a href="${CONTEXT_PATH}/msg/teacher/list?nowPage=${num}&keyword=${p.keyword}&type=${p.type}">${num}</a>
	                                	</span>
	                                </c:if>
                            	</c:forEach>
                                ...
                                <c:if test="${p.nowPage ne p.endPage}">
                                	<span class="jsgrid-pager-nav-button" style="cursor:pointer">
                                		<a href="${CONTEXT_PATH}/msg/teacher/list?nowPage=${p.nowPage + 1}&keyword=${p.keyword}&type=${p.type}">다음</a>
                                	</span>
                                </c:if>
                                ...
                                <c:if test="${p.nowPage ne p.lastPage}">
                           		<span class="jsgrid-pager-nav-button"><a href="${CONTEXT_PATH}/msg/teacher/list?nowPage=${p.lastPage}&keyword=${p.keyword}&type=${p.type}">끝</a></span>
								</c:if>
                            </div>
                        </div>
...
<script type="text/javascript">
	...
	$('#resetSrh').on("click", function(){
		location.href = "${CONTEXT_PATH}/msg/teacher/list";
	});
